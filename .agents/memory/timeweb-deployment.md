...
**How to apply:** Keep the existing service on 8080 untouched. Add the server's public deploy key to the GitHub repository, configure `TELEGRAM_BOT_TOKEN` in the server-only environment file, then use the prepared update script and verify `/api/healthz` plus Telegram bot connection logs.